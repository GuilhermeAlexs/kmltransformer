...
public class TrailsParserWithUCS implements KmlParseProgressListener {
	...
	public void onParsePlacemark(Placemark p) {
		try {
			TPLocation loc = KmlUtils.parsePlacemark(p, this);
	...
	public void onParseFinish(boolean altitudeWasDownloaded) {
		...
		try {
			...
			KmlUtils.parseKml(kml, new KmlParseProgressListener(){
				...
				public void onParsePlacemark(Placemark p) {
					try {
						finalUCList.addAll(KmlUtils.parsePlacemarkUC(p, this));
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
				...
				public void onParseFinish(boolean altitudeWasDownloaded) {
					try{
						...
						for(TPLocation loc: locs){
						    ...
						    writer.println(loc.getName() + "$" + loc.getLatitude() + "$" +
						    		loc.getLongitude() + "$" + loc.getUc() + "$" + loc.getType().getValue() + "$" + "1");
						}
